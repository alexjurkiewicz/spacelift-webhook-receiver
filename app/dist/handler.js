...
const validator = __webpack_require__(63)
const parse = __webpack_require__(285)
const redactor = __webpack_require__(53)
const restorer = __webpack_require__(56)
const { groupRedact, nestedRedact } = __webpack_require__(405)
const state = __webpack_require__(185)
const rx = __webpack_require__(28)
...
const { groupRestore, nestedRestore } = __webpack_require__(405)
...
const errSerializer = __webpack_require__(254)
const reqSerializers = __webpack_require__(923)
const resSerializers = __webpack_require__(225)
...
const flatstr = __webpack_require__(638)
const {
  lsCacheSym,
  levelValSym,
...
const { noop, genLog } = __webpack_require__(818)
...
const { version } = __webpack_require__(686)
...
const SonicBoom = __webpack_require__(948)
const flatstr = __webpack_require__(638)
const {
  lsCacheSym,
  levelValSym,
...
const fastRedact = __webpack_require__(801)
const { redactFmtSym, wildcardFirstSym } = __webpack_require__(273)
...
const format = __webpack_require__(346)
const { mapHttpRequest, mapHttpResponse } = __webpack_require__(846)
const SonicBoom = __webpack_require__(948)
const stringifySafe = __webpack_require__(445)
const {
  lsCacheSym,
  chindingsSym,
  parsedChindingsSym,
...
function getPrettyStream (opts, prettifier, dest, instance) {
  ...
  try {
    const prettyFactory = __webpack_require__(455)
...
const stdSerializers = __webpack_require__(846)
const redaction = __webpack_require__(584)
const time = __webpack_require__(85)
const proto = __webpack_require__(398)
const symbols = __webpack_require__(273)
const { assertDefaultLevelFound, mappings, genLsCache } = __webpack_require__(97)
const {
  createArgsNormalizer,
  asChindings,
  final,
...
const { version } = __webpack_require__(940)
const {
  chindingsSym,
  redactFmtSym,
  serializersSym,
...
const sleep = __webpack_require__(866)
 ...
 */
const verifySpaceliftEvent = (event, token) => {
    ...
    const bodyHash = (0,external_crypto_namespaceObject.createHmac)('sha256', token).update(body).digest('hex');
    const actualSignature = `sha256=${bodyHash}`;
    if (claimedSignature.length !== actualSignature.length ||
        !(0,external_crypto_namespaceObject.timingSafeEqual)(Buffer.from(claimedSignature), Buffer.from(actualSignature))) {
 ...
 * @returns
 */
async function lambdaEntry(raw_event) {
 ...
 * @returns
 */
function lambdaError(clientMessage, logData) {
    if (logData) {
